...
function dueDateOffsetMs(quality: number): number {
  if (quality === 0) return MIN;       // Again — 1 min
  if (quality <= 2)  return 5 * MIN;  // Hard  — 5 min
  if (quality <= 4)  return 15 * MIN; // Good  — 15 min
  return 5 * DAY;                     // Easy  — 5 days
}
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden" style={{ background: "#05050f" }}>
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-5">
        ...
        <div className="flex items-center gap-3">
          <button
           onClick={() => { setCardFlipped(true); setAuthMode("login"); cardRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); }}
            className="text-sm text-white/60 hover:text-white transition-colors px-4 py-2"
          >
            Sign In
          </button>
          <button
            onClick={() => { setCardFlipped(true); setAuthMode("signup"); cardRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); }}
      ...
      <div className="relative z-10 container mx-auto px-8 pt-16 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center min-h-[calc(100vh-80px)]">
        ...
        >
          ...
          >
            <button
              onClick={() => { setCardFlipped(true); setAuthMode("signup"); }}
      ...
      >
        ...
        >
          ...
          >
            ...
            <p className="text-xl font-semibold text-foreground text-center leading-relaxed overflow-y-auto max-h-[140px] w-full px-1 [&::-webkit-scrollbar]:hidden" style={{ fontFamily: "Outfit, sans-serif", fontSize: card.front.length > 120 ? "0.875rem" : card.front.length > 60 ? "1rem" : "1.25rem" }}>
  {card.front}
</p>
          ...
          >
            ...
            <p className="text-xl font-semibold text-foreground text-center leading-relaxed overflow-y-auto max-h-[140px] w-full px-1 [&::-webkit-scrollbar]:hidden" style={{ fontFamily: "Outfit, sans-serif", fontSize: card.back.length > 120 ? "0.875rem" : card.back.length > 60 ? "1rem" : "1.25rem" }}>
  {card.back}
</p>
          </div>
        </div>
      ...
      <AnimatePresence>
        {cardFlipped && (
          ...
          >
            {[
              { label: "Again", sub: "1 min", quality: 0, color: "#ef4444", bg: "rgba(239,68,68,0.12)", border: "rgba(239,68,68,0.25)" },
              { label: "Hard", sub: "5 min", quality: 2, color: "#f97316", bg: "rgba(249,115,22,0.12)", border: "rgba(249,115,22,0.25)" },
              { label: "Good", sub: "15 min", quality: 4, color: "#6366f1", bg: "rgba(99,102,241,0.12)", border: "rgba(99,102,241,0.25)" },
              { label: "Easy", sub: "5 days", quality: 5, color: "#34d399", bg: "rgba(52,211,153,0.12)", border: "rgba(52,211,153,0.25)" },
...
export default function App() {
 ...
 const rateCard = async (quality: number) => {
  ...
  const updatedCard = {
    ...
    repetitions,
    easeFactor,
    dueDate: Date.now() + dueDateOffsetMs(quality),
  };

  const deckCards = (allCards[card.deckId] ?? []).map(c => c.id === card.id ? updatedCard : c);
  setAllCards(prev => ({ ...prev, [card.deckId]: deckCards }));
  storage.localSaveCards(card.deckId, deckCards);
  if (user) await storage.fsSaveCard(updatedCard);
  ...
  if (quality <= 2) {
  // Remove the current card from its slot and push it to the end of the queue.
  // Because we remove at studyIndex, the card that was at studyIndex+1
  // shifts down into studyIndex — so we do NOT increment studyIndex here.
  setStudyCards(prev => {
    const without = prev.filter((_, i) => i !== studyIndex);
    return [...without, { ...updatedCard }];
  });
  setCardFlipped(false);
  // studyIndex stays the same — it now points at the next card automatically
} else {
  const nextIndex = studyIndex + 1;
